...
#   # Single table
#   docker exec -u root hdp-edge-node bash /test-discover.sh sales_db orders sales_db s3a://my-bucket
#
#   # All tables in a database
#   docker exec -u root hdp-edge-node bash /test-discover.sh hr_db * hr_db s3a://my-bucket
#
#   # Multiple specific tables
#   docker exec -u root hdp-edge-node bash /test-discover.sh sales_db orders,customers sales_db s3a://my-bucket
#
#   # With partition filter
#   docker exec -u root hdp-edge-node bash /test-discover.sh logs_db app_logs logs_db s3a://my-bucket dt>=2024-01-15
#
#   # Unregistered partitions table (triggers MSCK REPAIR TABLE path in DAG)
#   docker exec -u root hdp-edge-node bash /test-discover.sh analytics_db sessions analytics_db s3a://my-bucket
