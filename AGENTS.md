...
- **deploy.py rewrites DAG content in memory**: it string-replaces `dag_id` and `owner` before uploading, so DAG files must contain the exact markers defined in `PROJECTS` dict (e.g. `dag_id='iceberg_migration'`; for migrator DAGs the owner marker is the `return 'data-migration'` fallback inside `_resolve_dag_owner()` — the `migration_dag_owner` Airflow Variable, read at parse time, outranks it).
- **The DAG owner is an auth principal, not a label**: a `pod_mutation_hook` in `nx1-full-package` (`terraform/tenant/modules/airflow/airflow-values.yaml`) reads the `dag.owners` column and injects it into every task pod as `HADOOP_USER_NAME`/`SPARK_USER`, so `default_args['owner']` is what Ranger authorizes for direct S3/HDFS access. It is one value per DAG refreshed each parse, never per run — the per-run user is separate, resolved by `get_config()` into `spark.sql.kyuubi.session.user`.
