...
def discover_hive_tables(db_config: dict, spark) -> dict:
    ...
    try:
        tz_rows = spark.sql(f"""
            ...
            LIMIT 1
        """).collect()
        if tz_rows and tz_rows[0]["source_timezone"]:
            source_timezone = tz_rows[0]["source_timezone"]
            logger.info(f"[IcebergDiscover] Inherited source_timezone='{source_timezone}' from DAG 1 for {src_db}")
        else:
            logger.warning(f"[IcebergDiscover] No source_timezone found in DAG 1 tracking for {src_db}, defaulting to UTC")
    except Exception as e:
        logger.warning(f"[IcebergDiscover] Could not read source_timezone for {src_db}: {e}. Defaulting to UTC")

    # Apply it for all DESCRIBE queries in this task
...
def migrate_tables_to_iceberg(discovery: dict, dag_run_id: str, spark, **context) -> dict:
    ...
    source_timezone = discovery.get('source_timezone', 'UTC')

    spark.conf.set("spark.sql.session.timeZone", source_timezone)
    logger.info(f"[IcebergMigrate] Set session timezone to '{source_timezone}' for {src_db}")
...
def validate_iceberg_tables(migration_result: dict, spark, **context) -> dict:
    ...
    source_timezone = migration_result.get('source_timezone', 'UTC')

    spark.conf.set("spark.sql.session.timeZone", source_timezone)
    logger.info(f"[IcebergValidation] Set session timezone to '{source_timezone}' for {src_db}")
