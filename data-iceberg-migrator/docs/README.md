...
| `migration_distcp_bandwidth`       | _(empty)_        | Bandwidth limit per mapper (MB/s). Empty (default) auto-sizes; must be set together with `migration_distcp_mappers` — one alone is rejected at config resolution                                                 | `source_to_s3_migration`, `folder_only_data_copy` |
