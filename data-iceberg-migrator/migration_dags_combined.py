...
def _configure_spark_s3(spark, config: dict):
...
def _apply_bucket_credentials(spark, bucket_url: str, endpoint: str, access_key: str, secret_key: str):
...
def _compute_dest_path(source_location: str, dest_database: str, table_name: str,
...
def discover_source_hive_tables(db_config: dict, spark, **context) -> dict:
    ...
    _configure_spark_s3(spark, config)
...
def validate_data_presence(discovery: dict, spark, **context) -> dict:
    ...
    if failed:
        context['ti'].xcom_push(key='return_value', value=result_dict)
        raise Exception(
            f"[validate_data_presence] S3 connectivity/API errors for "
            f"{len(failed)}/{len(results)} table(s) in '{discovery['source_database']}' — "
